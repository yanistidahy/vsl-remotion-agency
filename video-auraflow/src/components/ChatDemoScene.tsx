...
import { AbsoluteFill, Audio, interpolate, staticFile, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { C, FONT } from "../constants";
import { ChatWidget } from "./ChatWidget";
import { KeyboardTyping } from "./KeyboardTyping";

const TYPING_TEXT = "j'ai les cheveux secs et frisés";
const FPC = 4; // frames per character
const CHARS = TYPING_TEXT.length; // 31

// Key timings (local frames)
...
const BOT_MSG2_START = 240;
const PRODUCT_START = 320;
const KEYBOARD_HIDE = USER_SEND + 20; // 238

// Map character index to AZERTY key name
// AZERTY rows: A Z E R T Y U I O P / Q S D F G H J K L M / W X C V B N
const AZERTY_CHAR_MAP: Record<string, string> = {
...
function charToKey(ch: string): string | null {
  return AZERTY_CHAR_MAP[ch.toLowerCase()] ?? null;
}
...
export const ChatDemoScene: React.FC = () => {
  ...
  // Step indicators — each lights up at corresponding event frame
  const stepTriggers = [BOT_MSG1_START, USER_TYPE_START, BOT_MSG2_START, PRODUCT_START];
  const steps = [
    { icon: "👋", label: "Accueil personnalisé" },
    { icon: "⌨️", label: "Analyse du besoin client" },
    { icon: "🎯", label: "Recommandation ciblée" },
    { icon: "🛒", label: "Produit + achat direct" },
  ];

  // Input text: typing in progress, clears after send
  ...
  function getActiveKey(): string | null {
    ...
    return charToKey(TYPING_TEXT[idx]);
  }

  // Keyboard visibility
  const keyboardProgress = (() => {
    if (frame < USER_TYPE_START) return 0;
    if (frame > KEYBOARD_HIDE) return Math.max(0, 1 - (frame - KEYBOARD_HIDE) / 15);
    return Math.min(1, (frame - USER_TYPE_START) / 15);
  })();
  ...
  if (frame >= USER_SEND && frame < USER_SEND + 5) {
    audios.push(<Audio key="notif" src={staticFile("notification.mp3")} volume={0.5} />);
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {steps.map((s, i) => {
            const active = frame >= stepTriggers[i];
            const dotOp = interpolate(frame, [stepTriggers[i], stepTriggers[i] + 12], [0.3, 1], { extrapolateRight: "clamp" });
            return (
              <div
                key={i}
                style={{
              ...
              >
                <div
                  style={{
                    ...
                    flexShrink: 0,
                    opacity: active ? dotOp : 1,
                  }}
                />
                <span style={{ fontSize: 14, color: active ? "#fff" : "rgba(255,255,255,0.4)", fontWeight: active ? 600 : 400 }}>
                  {s.icon} {s.label}
                </span>
              </div>
            );
